...
export default function CustomTUModal({ isOpen, onClose, onImport, playClickSound, playBackSound }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm outline-none border-none">
      <div className="relative w-[450px] p-8 flex flex-col items-center shadow-2xl" style={{ backgroundImage: "url('/images/frame_background.png')", backgroundSize: "100% 100%", imageRendering: "pixelated" }}>
        <h2 className="text-[#FFFF55] text-2xl mc-text-shadow mb-6 border-b-2 border-[#373737] pb-2 w-full text-center uppercase font-bold tracking-widest">Import Custom TU</h2>

        <div className="flex flex-col gap-5 w-full">
          <div className="flex flex-col gap-2">
            <label className="text-gray-300 text-sm mc-text-shadow uppercase tracking-widest ml-1">TU Name</label>
            <input
          ...
          <div className="flex flex-col gap-2">
            <label className="text-gray-300 text-sm mc-text-shadow uppercase tracking-widest ml-1">Description (Optional)</label>
            <input
          ...
          <div className="flex flex-col gap-2">
            <label className="text-gray-300 text-sm mc-text-shadow uppercase tracking-widest ml-1">Download URL (.zip)</label>
            <input
          ...
          {error && <div className="text-red-500 text-center mc-text-shadow uppercase text-xs tracking-widest mt-1">{error}</div>}
        </div>
        ...
        <div className="flex gap-4 mt-8 w-full">
          ...
          <button
            onClick={() => { playClickSound(); handleImport(); }}
            className="flex-1 h-12 flex items-center justify-center text-white text-xl mc-text-shadow transition-all outline-none border-none bg-transparent hover:text-[#FFFF55]"
            style={{ backgroundImage: "url('/images/Button_Background.png')", backgroundSize: '100% 100%', imageRendering: 'pixelated' }}
            onMouseEnter={(e) => e.currentTarget.style.backgroundImage = "url('/images/button_highlighted.png')"}
            onMouseLeave={(e) => e.currentTarget.style.backgroundImage = "url('/images/Button_Background.png')"}
